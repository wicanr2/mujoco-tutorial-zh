...
用 MuJoCo 物理引擎寫的 28 篇繁體中文教學、42 支 Python 腳本，以及一整條 AMR 叉車搬運實驗線 —
...
| [20 真實模型重跑](docs/06-amr/08-rerun-real-model.md) | 把 16/17 章搬到 2000 kg 真車上 | 低速橫移下木頭 0.9 / 塑膠 28.5 cm；強度一高兩者都甩落 |
...
| 方法 | 最終回報 | 環境步數 | 訓練時間 | 硬體 | 章節 |
| --- | --- | ---: | --- | --- | --- |
| ARS（手寫 numpy） | -0.30 | 480k | 18–21 秒 | CPU | [08](docs/02-programming/03-rl-swingup.md) |
| PPO（SB3） | -0.10 | 150k | 6 分鐘＊ | CPU | [09](docs/02-programming/04-ppo-gymnasium.md) |
| SAC（`train_freq=4`） | 未收斂 | 60k | 26 秒 | CPU | [12](docs/02-programming/07-sac.md) |
| SAC（`train_freq=1`） | -0.21 | 60k | 84 秒 | CPU | [12](docs/02-programming/07-sac.md) |
| SAC（zoo 超參） | -0.21 | 60k | 4 分鐘 | RTX Pro 6000（8 環境） | [12](docs/02-programming/07-sac.md) |
| SAC（同上，GPU 對照） | -0.20 | 60k | 68 秒 | RTX Pro 6000（4 環境） | [12](docs/02-programming/07-sac.md) |

![三種演算法的學習曲線](runs/rl_curves.png)

＊訓練時間都是在空閒機器上量的（load average < 1）。同樣的 PPO 150k 步，在 load average
超過 20 的機器上要 30 分鐘；CPU 版 SAC 的 60k 步更誇張，忙碌時要數小時、閒置時 38 秒。
**牆鐘時間要連機器狀態一起看**，否則會把鄰居的負載當成演算法的特性。

**訓練時間快不等於樣本效率高。** ARS 十幾秒就跑完，但它吃掉 48 萬個環境步 — 是 SAC 的
八倍。隨機搜索每一步都便宜（沒有神經網路、沒有梯度），代價是完全不重複利用資料。真機上
樣本很貴的時候，這個取捨會整個反過來。

**為了省時間砍掉的超參，要回頭驗它砍掉了什麼。** 12 章的 SAC 一開始沒學起來，當初歸因給
「CPU 上每步梯度更新太貴，所以 `train_freq` 只能調低」。實測把這個前提推翻了：一次更新
1.4 ms，`train_freq=1` 跑滿 60k 步 84 秒，全程 CPU。消融四組之後，主因就是那個被砍掉的
`train_freq`（-1.82 → -0.22），學習率只是次要。同一組超參拿去 GPU 跑是 68 秒 —— **只快
1.2 倍**，因為兩層 256 的網路填不滿 GPU。「RL 訓練要用 GPU」得看規模。訓練好的權重都收在
[`policies/`](policies/)，可以直接載入試玩。

## 幾個值得記下來的結論

**「這個平台測不出來」往往只是參數沒掃到。** 16 章用輕量車測得木頭與塑膠的滑動量差 30 倍
（0.4 cm vs 11.9 cm）；20 章搬到 2000 kg 真車上，一度得到「兩種材質都是 15.7 cm、分不出來」
的結論。實際上那是兩個問題疊在一起：叉齒的碰撞盒沒對準叉孔（變成撞著棧板推，而不是插進去
抬），以及橫移強度直接用了滿幅 ±1.2 m/s，一開始就跨過飽和點。修好對位、把強度掃一遍之後，
低速區（0.2 m/s）木頭 0.9 cm、塑膠 28.5 cm，差 27.6 cm — 比輕量平台更顯著。
**下「測不出來」的結論之前，先確認掃過該掃的範圍。**
...
**驗收條件寫什麼，就只驗到什麼。** 24 章的判定原本寫「棧板最終高度夠低」，於是貨在搬運
...
四十七條除錯案例的完整清單在 [REPORT.md](REPORT.md)，每一條在對應章節都有現象、原因與解法。
七輪稽核的完整紀錄（含更正了什麼、驗證過哪些仍正確）也在同一份文件。
...
2026-09-11 升版到 MuJoCo 3.13.0 時：42 支腳本跑了 39 支、全數通過，輸出與版控裡的位元相同
（例外是 24／25 章那兩組 —— 同一輪修掉了一個模型缺陷，數字跟著變）。沒跑的 3 支都要在
Blender 裡執行。三支 RL 腳本與需要 CUDA 的那支在 RTX Pro 6000 那台跑，逐項紀錄見
