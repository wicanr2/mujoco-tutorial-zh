...
| [24 貨架取放 X/Z](docs/06-amr/12-steer-reach-xz.md) | 舵輪底盤 + 環氧地板 + 層板取貨 | 取貨成立；搬運段貨滑落 0.55 m |
| [25 全軸取放 X/Y/Z](docs/06-amr/13-reach-xyz.md) | 加上 y 向 reach 側移 | 三軸機構成立；放置段未完成 |
...
**驗收條件寫什麼，就只驗到什麼。** 24 章的腳本判定寫的是「棧板最終高度夠低」，於是貨在
搬運途中從叉齒滑落、掉在半路上，判定照樣印出「驗證通過」。25 章更隱蔽：`drive_to` 的
`hold` 參數在退出前把門架重新升起，貨被舉到 1.25 m 再摔下來，而終點位置剛好滿足三個判定
條件。兩章的取貨都成立，搬運與放置不成立 — 文件已標明，斷言也改成只驗它真正證明的事。
判定實驗成功要看整段軌跡，不是最後一幀。
