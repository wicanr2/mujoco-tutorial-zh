...
3. **CI 會在乾淨的 checkout 上重跑**（`.github/workflows/verify.yml`）：靜態稽核
   ...
   `check_docs.py` 檢查十二項：絕對路徑、文件相對連結、模型可載入、文件裡的 MJCF 區塊、
   章節結構、章節編號與依賴順序、各目錄 README 的清單登記、孤兒圖片、相依套件登記、
   **模型接觸穿透**、README/REPORT 宣稱的數量、CSV 欄位一致。**新增檢查請加進這支腳本**，
   不要只在對話裡跑一次 —— 稽核要能重複執行才有意義。

   「模型接觸穿透」那項是這樣來的：機構與物件的幾何沒對好時，模擬照樣跑、判定照樣通過，
   只有量接觸才看得到（20 章的叉齒沒對準叉孔，全程 15 mm 穿透，變成「撞著推」而不是
   「插進去抬」）。**靜置後仍收不回去的穿透就是幾何沒對好。**
