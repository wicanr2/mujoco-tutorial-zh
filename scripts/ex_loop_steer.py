...
axes[1].plot(log[:, 0], np.rad2deg(log[:, 3])); axes[1].set_title("yaw (deg)"); axes[1].grid(True)
axes[2].plot(log[:, 0], log[:, 4], label="steer (rad)")
axes[2].plot(log[:, 0], log[:, 5], label="drive (rad/s)")
