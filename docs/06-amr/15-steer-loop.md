...
轉彎外擺：x 最遠 2.41（waypoint 2.0，超調 0.41 m）、y 最遠 2.38（超調 0.38 m）
...
軌跡是圓角正方形，轉彎時外擺 0.41 m（x 最遠到 2.41，waypoint 在 2.0；這個數字現在由腳本
自己算出來印在 stdout，不是從圖上讀的）— 舵輪不能原地轉，
路徑必然外擺，與 TB3 實驗中阿克曼車的結論一致。`wp_i` 從 0 起算，而 waypoint 0 就是車子的
起點 (0, 0) —— 車還沒動就算到達了，所以 stdout 印的「到達過 7 個」對應的是 6 段實際路程，
每段約 10 秒（t=3.0、15.3、25.4、35.7、46.0、56.3）。最後一個 waypoint 沒進到 0.35 m
容差內：切換目標後車子在 (0.3, 1.7) 附近以最大轉向角（1.39 rad）繞小圈，剩下的 94 秒
都沒能收斂進去。轉彎半徑
