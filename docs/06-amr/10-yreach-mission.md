...
> 擷取日期：2026-09-10
>
> 測試環境：Linux、MuJoCo 3.13.0、Python 3.12.3、imageio/matplotlib；範例已實測通過。
...
**漂移峰值 14.3 cm，是 [24 章](12-steer-reach-xz.md)（6.7 cm）的兩倍。** 差別在於這裡
...
5. **`render(out=...)` 會讓所有影格變成同一張**：`renderer.render()` 不帶參數時每次配置新陣列（MuJoCo 3.13.0 實測），直接 append 是安全的；但傳 `out=buf` 重用同一個陣列時，清單裡每個元素都指向 `buf`，最後全部是最後一張。範例保留 `.copy()` 是防禦性寫法 — dm_control 的 `physics.render()` 確實共用緩衝區，這個習慣從那裡沿用下來。
