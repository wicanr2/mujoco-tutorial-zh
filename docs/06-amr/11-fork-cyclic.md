...
2. **循環中棧板向叉尖蠕動 22 cm**：每次「下」到底的衝擊讓棧板一點點往前走。解法是真實叉車的標準做法 — **後傾（tilt +0.08 rad）把貨靠在滑架背板上**，漂移從 22 cm 降到 1.4 cm。
3. **滑動量測的基準**：「相對牙叉位置」是絕對值（取貨後約 -0.65 m），漂移量要拿「取貨完成時」當基準相減，不能直接對零點判斷。
