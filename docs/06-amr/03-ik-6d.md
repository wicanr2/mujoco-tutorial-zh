...
> ⚠️ **本章最大的坑**：一開始改用 `mju_subQuat`（軸角向量）當姿態誤差，怎麼調參數都不收斂。
> 實測對照（同一個模型、同一組目標，只換誤差定義）：
>
> | 姿態誤差定義 | 步長 0.2 | 步長 0.4 | 步長 0.8 |
> | --- | --- | --- | --- |
> | 四元數差的向量部分 | 76 次收斂 | 37 次收斂 | 18 次收斂 |
> | `mju_subQuat` | 200 次未收斂 | 200 次未收斂 | 200 次未收斂 |
>
> `mju_subQuat` 那組三種步長都卡在姿態誤差 ≈ π。本章的目標姿態剛好在初始姿態的 180° 對面，
> 而 `mju_subQuat` 在這個奇異點附近給的方向與 `jacr` 的線性化不相容，迭代被推向反方向出不來。
> 小角度時兩者方向一致、只差一個 2 倍尺度，所以問題只在大角度浮現。官方 Python 教學用的
> 就是上面的「四元數差向量部分」寫法。**姿態誤差不要自己發明，用官方配方。**
