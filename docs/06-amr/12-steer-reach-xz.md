...
搬運高度棧板 z = 0.974
放下後棧板 = [ 0.601  0.002 -0.   ]
棧板被搬運了 2.10 m；搬運全程在牙叉座標系中的漂移峰值 6.7 cm
結果：取貨 → 搬運 → 放置 全程驗證通過 ✓
```
...
配合後傾 0.08 rad 與保守的速度上限（線速度約 0.23 m/s），搬運全程的漂移**峰值 6.7 cm**，
貨還在叉齒上的最後一刻收斂到 0.1 cm。
...
> 1.4 cm。這裡多了行進，同樣的手法把 110 cm 壓到 0.1 cm。**貨靠上滑架背板，是叉車載貨
> 移動的前提。**
...
7. **航向控制的目標值要對著機構的朝向設**。牙叉在車體 -x 側，目標航向就該是 0 而不是 π。
   ...
   很容易誤診成驅動力不足或摩擦問題。
8. **機構撞自己不會有任何提示**。第 1 條把 base↔swivel/drive 排除掉之後，門架滑台與前輪
   的碰撞盒卻一直沒人管：`stage` 越過 **-0.683 m** 就開始重疊，滑到行程底穿透 17 mm。表面
   完全正常 — 腳本照樣印驗證通過、影片看起來也對，只有量接觸力才看得到那一瞬間的
   **28 kN 內力**，而它把整台車往後推了 2.6 cm（`drive_to` 停在 -0.603 而不是 -0.629）。
   叉齒滑台也一樣：低位時 `reach_y` 落在 **±0.144 ～ ±0.344 m** 這段會掃進輪子（再遠就
   從輪子外側滑過去了）。兩組都補上 `<contact><exclude>`。**同一條教訓要掃過整個模型**，排除了一對不代表其他對是安全的；
   這種只在特定關節位置才出現的干涉，靜置檢查抓不到，`check_docs.py` 的「機構自我干涉」
   就是為此加的（逐關節走過 range 找機構自己撞自己的零件對）。
