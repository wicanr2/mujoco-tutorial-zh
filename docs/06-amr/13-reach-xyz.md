...
放下後棧板 z = -0.000
最終棧板 = [ 0.551  0.004 -0.   ]
棧板被搬運了 2.05 m；全程在牙叉座標系中漂移 3.3 cm
...
**側移的傳遞比 24 章的直線行進鬆一些**：全程漂移 3.3 cm，而純直線的 24 章是 1.0 cm。
reach 滑台橫移時貨要靠摩擦跟著走，橫向加速度直接作用在接觸面上，沒有背板可靠 —— 後傾
擋得住前後方向的滑動，擋不住側向。要更準的側向對位，得放慢 reach 的速度，或接受這個
量級的誤差、事後用量測修正。這與 16、20 章的摩擦實驗結論一致。
