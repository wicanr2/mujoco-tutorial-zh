...
放下後棧板 z = -0.000
最終棧板 = [ 0.551  0.004 -0.   ]
棧板被搬運了 2.05 m；搬運全程在牙叉座標系中的漂移峰值 12.1 cm
...
**側移的傳遞比 24 章的直線行進鬆一些**：漂移峰值 12.1 cm，而純直線的 24 章是 6.8 cm
（兩者到終點都收斂到個位數 cm）。reach 滑台橫移時貨要靠摩擦跟著走，橫向加速度直接作用在
接觸面上，**沒有背板可靠** —— 後傾擋得住前後方向的滑動，擋不住側向。要更準的側向對位，
得放慢 reach 的速度，或接受這個量級的誤差、事後用量測修正。這與 16、20 章的摩擦實驗結論
一致：摩擦傳遞在加速段一定會打折，差別只在打多少。
...
- 24 章的七條除錯紀錄在本章同樣適用，尤其是深插不收回、先退出貨架再升高、航向目標要對著
  機構朝向設這三條。
