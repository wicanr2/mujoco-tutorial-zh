...
[![雙擺擺動](../assets/strip_double_pendulum.png)](../assets/strip_double_pendulum.png)

從 `qpos = [1.0, 1.0]` 放開之後的四個時間點。兩節很快就失去規律 — 這就是混沌：
初始條件差一點點，幾秒後的姿態就完全不同。背景是黑的因為這個模型只有一盞燈、
沒有地板，刻意保持最小。

> 測試環境：Linux、MuJoCo 3.13.0、Python 3.12.3；實測輸出 `qpos = [-0.649 -0.385]`、`qvel = [2.251 0.812]`（混沌系統，小數位可能因版本略有差異）。
