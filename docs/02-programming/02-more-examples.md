...
> 擷取日期：2026-09-09
>
> 測試環境：Linux、MuJoCo 3.13.0、Python 3.12.3；本章所有範例皆已實測，輸出為實際執行結果。
...
實測輸出：16 條 rollout 全部完成，最終 qpos 分布各異（雙擺混沌特性）。

[![07 章三個範例的量測](../../runs/more_examples.png)](../../runs/more_examples.png)

三個範例各自留下 CSV，畫成圖之後看得到數字看不出來的東西：

- **觸覺感測器**（左）：球落地瞬間 touch 讀值衝到 1000 N 以上，只持續幾個 timestep。
  接觸力是尖峰不是台階 — 用低取樣率記錄很容易整個錯過，所以這支腳本以 200 Hz 取樣。
- **position 伺服**（中）：追正弦軌跡時實際角度在目標附近上下振盪，而且整體落後。
  伺服增益固定時，目標變化越快、相位落後越多。
- **平行 rollout**（右）：16 條軌跡在關節角平面上的走向。初始角度是均勻隨機取的，
  1 秒之後彼此完全發散 — 這是雙擺的混沌特性，也是為什麼 RL 訓練需要大量取樣。這個模式正是 RL 蒐集訓練資料的基礎；更大規模時執行緒會先撞到 GIL，改用批次向量化的 [MJX](08-mjx-gpu.md)（28 章有三種做法的吞吐對照）或 MuJoCo Warp。
