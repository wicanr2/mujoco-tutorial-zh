...
> 擷取日期：2026-09-09
>
> 測試環境：Linux、MuJoCo 3.13.0、Python 3.12.3；已實測，約 21 秒完成 30 輪訓練。
...
約 18–21 秒完成訓練（視機器負載）；訓練好的權重存為 `policies/swingup_policy.npy`，
每輪的評估回報寫進 [runs/rl_ars_log.csv](../../runs/rl_ars_log.csv)。

[![三種演算法的學習曲線](../../runs/rl_curves.png)](../../runs/rl_curves.png)

同一個任務、三種做法放在一起看（PPO 見 [09 章](04-ppo-gymnasium.md)、SAC 見
[12 章](07-sac.md)）。x 軸換算成**環境步數**才有得比 —— ARS 數的是輪，一輪要跑
`2 × N_DIR = 16` 個 episode、每個 1000 步，所以 30 輪等於 48 萬個環境步。

這帶出一件從「訓練時間」看不出來的事：**ARS 十幾秒就跑完，但它吃掉的樣本是 SAC 的
八倍**。隨機搜索每一步都很便宜（沒有神經網路、沒有梯度），所以牆鐘時間短；但它完全
不重複利用資料，樣本效率是三者裡最低的。真機上樣本很貴的時候，這個取捨會反過來。
