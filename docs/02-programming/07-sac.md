...
> 測試環境：Linux、Python 3.12.3、MuJoCo 3.13.0、sb3 2.9.0、torch 2.14.0（**CPU**）。
...
> 這一組是一次取樣的數字。SAC 在 CPU 上會做上萬次梯度更新，套件組合稍有變動就會挪動最後
> 幾位小數 —— 用 MuJoCo 3.12.0 與 3.13.0 各重跑一次前 3000 步，兩邊都得到 -1.9996（而不是
> 上表的 -1.9972），但「60k 步內完全沒離開躺平」這個結論每次都一樣。**負面結果要看的是
> 結論穩不穩，不是小數點後第四位。** 兩版都給同一個新值，也說明這個差別不是升版造成的。
